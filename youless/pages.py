...
def init_pages(web, coordinator, db):
    ...
    if hasattr(sys, 'frozen'):
        web.putChild("youless_images", File(os.path.join(os.path.dirname(sys.executable), 'plugins/youless/templates/images')))
    else:
        web.putChild("youless_images", File(os.path.join('houseagent/plugins/youless/templates/images')))
    web.putChild('youless_devices', YoulessDevices(coordinator, db))
    web.putChild('youless_devices_view', YoulessDevicesView(db, coordinator))


class YoulessDevicesView(Resource):
    ...
    def finished(self, locations):
        ...
        if hasattr(sys, 'frozen'):
            ...
            template = Template(filename=os.path.join(os.path.dirname(sys.executable), 'plugins/youless/templates/devices.html'), lookup=lookup)
        else:    
            lookup = TemplateLookup(directories=['houseagent/templates/'])
            template = Template(filename='houseagent/plugins/youless/templates/devices.html', lookup=lookup)
    ...
    def render_GET(self, request):
        ...
        # Uglyness alert!
        plugins = self.coordinator.get_plugins_by_type("Youless")
        if len(plugins) == 0:
            self.request.write(str("No online Youless plugins found..."))
            self.request.finish()
        elif len(plugins) == 1:
...
class YoulessDevices(Resource):
    ...
    def render_GET(self, request):
        ...
        if len(plugins) == 0:
            ...
            self.pluginguid = plugins[0].guid
            self.pluginid = plugins[0].id
            self.coordinator.send_custom(plugins[0].id, "get_devices", '').addCallback(self.result)   
